...
# БОГАТЫРСКИЙ КАМЕНЬ
решение = ввод("введите направление:").ниже()
если решение = = "левый":
    print("потерять коня")
 решение elif = = "правильно":
    print("потерять жизнь")
elif decision = = "прямой":
    распечатать("Lose mind")


story_stone = {
    "левый":"потерять лошадь",
    "правильно": "потерять жизнь",
    "прям": "теряю рассудок"
}
решение = ввод("введите направление:").ниже()
print(story_stone[решение])
print(story_stone.get(decision)) # ВАЖНО ПОНИМАТЬ РАЗНИЦУ
print(story_stone.ключи())
print(story_stone.ценности())
print(story_stone.предметы())
...
попробуй:
    1 / 0
кроме исключения как err:
    print("ошибка!!!", err)

а = 1
b = 2
# ТЕРНАРНЫЙ ОПЕРАТОР
результат = 123 if a == 3 else 5
печать(результат)
